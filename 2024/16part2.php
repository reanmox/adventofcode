...
foreach ($results as $result) {
    if ($result['score'] == $minScore) {
        foreach ($result['moves'] as $move) {
            $allPositions[] = $move[0] . ";" . $move[1];
        }
    }
}

$uniquePositions = array_unique($allPositions, SORT_REGULAR);

echo "Paths found: " . count($results) . "\n";
echo "Canceled paths: $canceledPaths\n";
echo "Part 1: shortest path: " . $minScore . "\n";
echo "Part 2: unique positions: " . count($uniquePositions) . "\n";
